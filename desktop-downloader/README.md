...
5. Let Rekordbox analyze BPM, waveform, key, beatgrid, and cues, then use Rekordbox’s own Device Library export for USB media.
...
7. File tags Rekordbox **does** read on import: **Genre** (from Beatport when available), **Grouping**, and **Comments**. The playlist name is written to Grouping and Comments; the year-month crate (`2026-09`) is included when year/month folders are enabled. Rekordbox **My Tags** and colour labels are stored only in Rekordbox’s database — they cannot be written into MP3/M4A/FLAC files or `.m3u8` playlists. After import, use Rekordbox’s own My Tag / colour tools, or **Reload Tags** if you re-export files that are already in the collection.
